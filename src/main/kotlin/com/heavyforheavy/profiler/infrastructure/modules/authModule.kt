  ...
  routing {
    ...
    route(Routes.AUTH_SIGN_UP) {
      ...
      val resultUser = signUpUseCase.signUp(userWithEmailAndPassword) {
        userWithEmailAndPassword.email?.let { it1 -> simpleJwt.sign(it1) }
    ...
    route(Routes.AUTH_SIGN_OUT) {
      val currentUser = call.getUserIdPrincipal()?.name?.let { email ->
        getUserByEmailUseCase.getUser(email)
      } ?: throw AuthException.InvalidToken()
      val newToken = currentUser.email?.let { mail ->
        signOutUseCase.signOut(mail) {
          simpleJwt.sign(currentUser.email)
        }
      } ?: throw AuthException.InvalidToken(message = "failed to create new token")
