...
fun Routing.userServicesRouting() {
    ...
    route(Routes.USER_AND_SERVICES_BY_ID) {
        call.respond(getUserAndServicesUseCase.getServices(call.requireParameter(Param.USER_ID).toInt()).response())
    }
    ...
    route(Routes.USER_SERVICES_BY_ID) {
        call.respond(getServicesUseCase.getServices(call.requireParameter(Param.USER_ID).toInt()).response())
    }
    ...
    route(Routes.DELETE_SERVICE) {
        call.respond(deleteServiceUseCase.deleteService(call.requireParameter(Param.SERVICE_ID).toInt()))
    }
}